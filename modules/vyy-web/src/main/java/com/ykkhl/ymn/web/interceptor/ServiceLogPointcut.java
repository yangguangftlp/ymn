...
public class ServiceLogPointcut implements MethodInterceptor, ThrowsAdvice {
	...
	private static final Logger LOGGER = Logger.getLogger(ServiceLogPointcut.class);
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
		...
		LOGGER.debug("Servie层开始执行【" + info + "】...参数：" + JSONObject.toJSONString(invocation.getArguments()));
		// 性能 测试
		SystemConfigCache<Object> systemConfigCache = SystemCacheUtil.getInstance().getSystemConfigCache();
		SystemConfig systemConfig = (SystemConfig) systemConfigCache.getSystemConfig("system", "performance.test");
		Object object = null;
		if (null != systemConfig) {
			// 开启测试
			if ("1".equals(systemConfig.getValue())) {
				long start = System.currentTimeMillis();
				LOGGER.debug("方法开始执行时间【" + DateUtil.dateToString(new Date(start), "yyyy-MM-dd HH:mm:ss.SSS") + "】...");
				object = invocation.proceed();
				long end = System.currentTimeMillis();
				LOGGER.debug("方法结束执行时间【" + DateUtil.dateToString(new Date(end), "yyyy-MM-dd HH:mm:ss.SSS") + "】执行时长【"
						+ (end - start) + "】毫秒...");
			} else {
				object = invocation.proceed();
			}
