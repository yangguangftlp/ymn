...
public class AES {
	public static void main(String[] args) {
		...
		// 解密 ——账号/身份证号
		byte[] accountFrom = AES.parseHexStr2Byte(encryptResultStr);
		byte[] deAccount = AES.decrypt(accountFrom, addPwd);
		String userAccount = new String(deAccount);
		System.out.println("解密后content：" + userAccount);
		// 解密——密码
		byte[] pwdFrom = AES.parseHexStr2Byte(parseByte2HexStr2);
		byte[] deUserPwd = AES.decrypt(pwdFrom, addPwd);
