        ...
        */
        that.show = function (prevAnim) {
            ...
            var html = '<div class="dw-trans ' + s.theme + ' dw-' + s.display + '">' +
                (s.display == 'inline' ? '<div class="dw dwbg dwi">' +
                '<div class="dwwr">' : '<div class="dw-persp">'
                + '<div class="dwo">' +
                '</div><div class="dw dwbg ' + mAnim + '">' +
                '<div class="dw-arrw"><div class="dw-arrw-i">' +
                '<div class="dw-arr"></div>' +
                '</div></div><div class="dwwr">' + (s.headerText ? '<div class="dwv"></div>' : ''));
            ...
            for (i = 0; i < s.wheels.length; i++) {
                ...
                if(s.wheels.length > 1){
                    _minWidth=parseInt(windowW/5);
                    console.log(_minWidth);
                }else{
                    var _num=0;
                    for(var js2 in s.wheels[0]){
                        _num++;
                    }
                    _minWidth=parseInt(windowW/_num);
                }
                for (label in s.wheels[i]) {
                    warr[l] = s.wheels[i][label];
                    html += '<td><div class="dwwl dwrc dwwl' + l + '">' +
                        (s.mode != 'scroller' ?
                        '<div class="dwwb dwwbp" style="height:' + hi + 'px;line-height:' + hi + 'px;"><span>+</span></div><div class="dwwb dwwbm" style="height:' + hi + 'px;line-height:' + hi + 'px;"><span>&ndash;</span></div>' : '') + '<div class="dwl">' + label + '</div>' +
                        '<div class="dww" style="height:' + (s.rows * hi) + 'px;min-width:' + _minWidth + 'px;">' +
                        '<div class="dw-ul">';
                    // Create wheel values
