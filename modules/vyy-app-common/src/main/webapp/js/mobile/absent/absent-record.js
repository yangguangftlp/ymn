...
var listModule= 0,//0：只有状态选择，1：只有搜索，2：机油状态又有搜索
	...
	detailUrl = "mobile/absent/absentDetailView.action",
	dataUrl = ["mobile/absent/getAbsentRecord.action?time="+new Date().getTime(),"mobile/absent/getAbsentRecord.action?time="+new Date().getTime(),"mobile/absent/getAbsentRecord.action?time="+new Date().getTime()];
$(function(){
	//基础参数
		...
		if(_operatingStatus == 0){//加载
			if (!!data) {
				...
				if (size > 0) {
					...
					auditList
						.append("<div class='noRecord whiteBg'>没有记录......</div>");
				}
			} else {
				auditList
					.append("<div class='noRecord whiteBg'>没有记录......</div>");
