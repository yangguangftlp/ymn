...
var listModule= 2,//0：只有状态选择，1：只有搜索，2：机油状态又有搜索
    ...
    unExpenseUrl="mobile/expense/getAuditRecord.action?operationType=1&time="+new Date().getTime(),//tab切换，未报销的请求url
    expenseUrl="mobile/expense/getAuditRecord.action?operationType=2&time="+new Date().getTime(),//tab切换，已报销的请求url
    detailUrl="mobile/expense/auditDetailView.action",//列表详情url
    dataUrl=[unExpenseUrl,expenseUrl];
$(function(){
    //基础参数
    basicTransferData();
    //时间选择
...
function getFilterData(_searchData){
    ...
    if(sessionStorage.status){
        ...
        if(_status == 0){
            _searchUrl=dataUrl[0];
        }else{
            _searchUrl=dataUrl[1];
        }
    }else{
        if(window.Current_Meeting_Type == 0){
            _searchUrl=dataUrl[0];
...
function initSearchData(_filterConditions){
    ...
    $.trim($("#userName").val(_filterConditions.userName));//人员姓名
    $.trim($("#number").val(_filterConditions.expenseNum));//报销单号
}
//初始化页码
function initPage(){
    window.Current_Meeting_Not_PageIndex=2;
...
function lookDetail(){
    var _flag = "";
    if(window.location.href.indexOf("auditType=2") > 0){
        _flag = "&finance";
    }
    $(document).on("click","section li",function(){
        ...
        if(_status == "退回"){
            ...
            window.location.href=detailUrl+"?id="+$(this).attr("dataid")+_flag;
        }
    });
}
//时间校验
...
function selectTime(){
    ...
    $("#endDate,#startDate").mobiscroll($.extend(opt['date'], opt['default']));
}
//特殊的请求字段
