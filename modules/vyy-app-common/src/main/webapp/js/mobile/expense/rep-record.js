...
var listModule= 0,//0：只有状态选择，1：只有搜索，2：机油状态又有搜索
    ...
    dataUrl = ['mobile/expense/getExpenseRecord.action?operationType=1','mobile/expense/getExpenseRecord.action?operationType=2'];
$(function(){
    var chaining=$("input#go-url").val();
    //基础参数
    basicTransferData();
    //判断是否滚动到底部，加载更多数据
    judgeScroll(dataUrl);
...
function judgeScroll($dataUrl){
    $(document).scroll(function(){
        if($.fn.endlessScroll(document)){
            if(!isLoadIng){
                if(!isSlideBottom){
                    var pageIndex = window.Current_Meeting_Type==0 ? window.Current_Meeting_Not_PageIndex : window.Current_Meeting_Yes_PageIndex;
                    var _data = {
                        pageIndex: pageIndex,
