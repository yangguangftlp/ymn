...
var listModule= 0,//0：只有状态选择，1：只有搜索，2：机油状态又有搜索
    ...
    unDealWithUrl="mobile/appraisal/getAuditAppraisalRecord.action?time="+new Date().getTime(),//?operationType=0tab切换，未结束的请求url
    DealWithUrl="mobile/appraisal/getAuditAppraisalRecord.action?time="+new Date().getTime(),//?operationType=1tab切换，已结束的请求url
    detailUrl="mobile/appraisal/auditDetailView.action",//待处理url
...
function additionRecord(data,size,auditList){
    for (var i = 0; i < size; i++){
        ...
        var html='<li class="twoLine" dataid='+data[i].id+'>' +//id
            '<header>' +
