...
var listModule= 2,//0：只有状态选择，1：只有搜索，2：既有状态又有搜索
    ...
    unReceiveUrl = "mobile/courier/getMyCourierList.action",//未领取的链接地址
    collectedUrl = "mobile/courier/getMyCourierList.action",//已领取的链接地址
    detailUrl = "mobile/courier/courierDetailView.action",
            ...
            if (!!_data) {
                ...
                if (size > 0) {
                    for (var i = 0; i < size; i++) {
                        ...
                        html='<li class="twoLine" dataid='+_data[i].id+' flowType='+_data[i].flowType+'>'+
                            '<header>'+
                            '<span class="mainColor">'+_data[i].belong+'</span>'+
                            $str+
