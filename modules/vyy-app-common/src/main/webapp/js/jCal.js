					...
					for (var di = 0, ds = $(e.data._target).data('days'); di < ds; di++) {
						...
						if ( e.type == 'mouseover' )		{
							$(currDay).addClass('overDay');
								$(".jCal-input",currDay).width("100%").siblings("span").hide();
						}
						else if ( e.type == 'mouseout' )	{
							$(currDay).stop().removeClass('overDay');
							$(".jCal-input",currDay).width("90%").css({"border":"none"}).blur().siblings("span").show();
						}
						else if ( e.type == 'click' )		{
							$(currDay).stop().addClass('selectedDay');						
							if($(e.target).attr("class") == "jCal-input"){
								$(".jCal-input",currDay).width("100%").css({"border":"solid 1px #ccc"}).siblings("span").hide();
								return false;
							}
						}
