...
(function($) {
	var eSelectPeople = {
        loadDeptInfo : function loadDeptInfo(elParent, callback) {
        	...
        	_self.loadDate('mobile/common/getDepts.action?time='+new Date().getTime(),function(data){
                ...
                if(data.length == 1){
                	if(data[0].id == '1' && data[0].child != null){
                		data = data[0].child;
                	}
                }
        ...
        depListHeight:function(){
        	var that = this;
            var winHeight = that.config.wheight;
        ...
        printDep:function(data,parent){
            ...
            for(var i=0;i < depLen;i++){
                if(data[i].child){//有子级
                    ...
                    $(li).append("<div class='dep'><a href='javascript:;' class='depName'><i class='fa fa-caret-right'></i>"+ data[i].name +"</a><a href='javascript:;' class='getIn' data-id='"+ data[i].id+"'><i class='fa fa-angle-right'></i></a></div>").append("<ul></ul>").appendTo(parent);
                    _self.printDep(data[i].child, $(li).children("ul"));
                }else{//无子级
                    $("<li class='noChild'></li>").append("<div class='dep'><a href='javascript:;' class='depName'>"+ data[i].name +"</a><a href='javascript:;' class='getIn' data-id='"+ data[i].id+"'><i class='fa fa-angle-right'></i></a></div>").appendTo(parent);
                }   
            } 
        },
        printPeople:function(data,parent){
            ...
            for(var i=0;i < depLen;i++){    
                    if(data[i].position == null){
                        ...
                        info = "<div class='name'>"+ data[i].name +"</div><div class='pos'>"+ data[i].position +"</div>"; 
                    }
                    ...
                    $("<li></li>").append("<div class='people' id='"+ data[i].userid +"'><div class='ckBox'><input type='checkbox' name='"+ data[i].userid +"'><i class='fa fa-circle-o'></i></div>"+ pic + info +"</div>").appendTo(parent);   
            }
        },
        handleTree:function(){
            ...
            wrap.on("click",".depName",function(){
                ...
                if(thisChild.is(":visible")){
                    ...
                    thisClass.removeClass("fa-caret-down").addClass("fa-caret-right");
                }else{
                    thisChild.css("display","block");
                    thisClass.removeClass("fa-caret-right").addClass("fa-caret-down");
                }
            });
        },
        handlePopSearchResult:function(){
            ...
            if(isMultiselect == "true"){
                searchResult = '<div class="searchResult"><div class="depPeopleList"></div><div class="btnTool mult"><div id="JselAll" class="checkAll"><input type="checkbox"/><i class="fa fa-circle-o"></i>全选</div><a href="javascript:;" class="btn-ok">确定</a><a href="javascript:;" class="btn-cancel">取消</a></div></div>';
            }else{
                searchResult = '<div class="searchResult"><div class="depPeopleList"></div><div class="btnTool"><a href="javascript:;" class="btn-ok">确定</a><a href="javascript:;" class="btn-cancel">取消</a></div></div>';
            }
        ...
        handleSearch:function(){
            ...
            $btn.on("click",function(){
                ...
                if(mumberData.length == 0){
                    //公司id=1
                	_self.loadDate('mobile/common/getDeptUser.action?deptId=1&time='+new Date().getTime(),_self.printSearchResult);
                }else{
        ...
        handleSelMumber:function(){
            ...
            wrap.on("click","#JselAll",function(){
                ...
                if(!isChecked){
                    ...
                    thisClass.attr("class","fa fa-check-circle");
                    ckBox.prop("checked",true);
                    ckClass.attr("class","fa fa-check-circle");
                }else{
                    allCheckBox.prop("checked",false);
                    thisClass.attr("class","fa fa-circle-o");
                    ckBox.prop("checked",false);
                    ckClass.attr("class","fa fa-circle-o");         
                }
            });
            //选择按钮
            wrap.on("click",".people",function(e){
                ...
                if(isMultiselect == "true"){
	                ...
	                if(ckNum == ckSize){
	                    ...
	                    allCkBox.siblings("i").attr("class","fa fa-check-circle");
	                }else{
	                    allCkBox.prop("checked",false);
	                    allCkBox.siblings("i").attr("class","fa fa-circle-o");
	                }
                }else{
                	var ckBox = $(".ckBox").find("input[type='checkbox']"),
                    	ckClass = $(".ckBox").find("i");
	...
	$.fn.eSelectPeople = function(config) {
        ...
        var closeBtn = $("<button type='button'>关闭</button>");
        var loading = $("<div class='load' id='loading' style='display:none'><i class='fa fa-spinner'></i></div>");
        var search = $('<div class="contactSearch"><input type="text" placeholder="姓名" id="JsearchInput"><i class="fa fa-search"></i><input type="submit" value="搜索" id="JsearchBtn"/></div>');
