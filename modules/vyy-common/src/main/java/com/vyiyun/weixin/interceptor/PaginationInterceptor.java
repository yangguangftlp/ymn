...
public class PaginationInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (obj instanceof SqlQueryParameter) {
			...
			if (sqlQueryParameter.isPage()
					&& sqlQueryParameter.getPageIndex() > 0
		...
		LOGGER.debug("the sql is :" + boundSql.getSql() + ",parameter is "
				+ obj);
	 ...
	 *            当前的数据库连接
	 */
	private void setTotalRecord(SqlQueryParameter sqlQueryParameter,
		...
		try {
			...
			BoundSql countBoundSql = new BoundSql(
					...
					parameterMappings, sqlQueryParameter);
			ParameterHandler parameterHandler = new DefaultParameterHandler(
					mappedStatement, sqlQueryParameter, countBoundSql);
	...
	private String getCountSql(String sql) {
		StringBuffer sqlBuffer = new StringBuffer(sql);
		int index = (index = sqlBuffer.indexOf("from")) > 0 ? index : sqlBuffer
				.indexOf("FROM");
		if (index != -1) {
	 ...
	 * @return Mysql数据库分页语句
	 */
	private String getMysqlPageSql(SqlQueryParameter page,
	 ...
	 */
	private String getOraclePageSql(SqlQueryParameter page,
			StringBuffer sqlBuffer) {
