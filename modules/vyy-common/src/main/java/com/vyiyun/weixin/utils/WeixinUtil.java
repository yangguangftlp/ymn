...
public class WeixinUtil {
	...
	public static JSONObject getSignature(String url, String params) throws Exception {
		// 取得配置项
		String corpId = HttpRequestUtil.getInst().getCurrentCorpId();// ConfigUtil.get(Constants.WEIXIN_APP_PATH,
																		// "corpId");
		// 生成ticket
		...
		LOGGER.debug("获取签名====url:" + url + "响应结果:" + jo);
		return jo;
	}
	...
	public static JSONObject getUserDetail(String userId) throws Exception {
		...
		return HttpsUtil.post(url, "access_token=" + getAccessToken() + "&userid=" + userId);
	}
	 ...
	 * @throws Exception
	 */
	public static JSONObject sendMessage(String msgBody, String accessToken) throws Exception {
		String url = ConfigUtil.get(Constants.WEIXIN_APP_PATH, "send_message_url");
		url += "?access_token=" + accessToken;
		LOGGER.debug("send message url is " + url + ",msgBody=" + msgBody);
		JSONObject responseData = HttpsUtil.post(url, msgBody);
	 ...
	 * @return
	 */
	public static JSONObject getWXAuthUser(String code) throws Exception {
		String url = ConfigUtil.get(Constants.WEIXIN_APP_PATH, "user_info_url");
		String accessToken = getAccessToken();
		return HttpsUtil.post(url, "access_token=" + accessToken + "&code=" + code);
	}
	...
	public static String getAccessToken() throws Exception {
		...
		String suiteSecret = HttpRequestUtil.getInst().getCurrentSuiteSecret();
		return getAccessToken(corpId, suiteId, suiteSecret);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static String getAccessToken(String corpId, String suiteId, String suiteSecret) throws Exception {
		String key = corpId+"_"+suiteId + "_access_token";
		// ICache<Object> iCache =
		// SystemCacheUtil.getInstance().getWeixinContactCache();
		LOGGER.error("获取企业getAccessToken,key：" + key);
		Map<String, Object> current = (Map<String, Object>) MemcacheUtil.getIns().getMemCached().get(key);
		JSONObject jsonObject = null;
		if (null != current) {
			jsonObject = new JSONObject(current);
		}
		// 若未缓存或缓存中的token还有5分钟过期，则重新获取token
		if (null == jsonObject || StringUtil.isEmpty(jsonObject.getString("access_token"))
				|| !jsonObject.containsKey("time")
				|| ((((JSONObject) jsonObject).getDate("time").getTime() + 7200000) - new Date().getTime()) <= 300000) {
			SystemConfig systemConfig = (SystemConfig) SystemCacheUtil.getInstance().getSystemConfigCache()
					.getSystemConfig("system", "getAccessToken");
			LOGGER.error("获取企业getAccessToken配置信息!" + systemConfig);
			JSONObject jo = JSON.parseObject(HttpsUtil.sendGet(MessageFormat.format(systemConfig.getValue(),
					new Object[] { corpId, suiteId, suiteSecret })));
			if (null != jo && "true".equalsIgnoreCase(jo.getString("success"))) {
				// 加入缓存
				JSONObject jsonData = jo.getJSONObject("data");
				if (null != jsonData && jsonData.containsKey("access_token")) {
					MemcacheUtil.getIns().getMemCached().add(key, jsonData);
					return jsonData.get("access_token").toString();
				}
			}
			LOGGER.error("获取企业token失败!" + jo);
			throw new VyiyunException("获取企业token失败!" + jo);
		} else { // 否则，直接从缓存中取出返回
